...
- **Tabs themselves are minimal**: plain text, no favicon, no background block, no border. The currently active tab uses the accent color (`text-primary`, i.e. Cloudflare orange-red `#ea580c`, already defined as `--primary` in `shared/ui/globals.css` — no new color needed); inactive tabs are neutral gray. The font is a large sans-serif with a condensed width (`font-stretch-extra-condensed`) plus `tracking-tight`, so hostnames can be shown tightly packed within limited width. Pinned and live tabs are rendered by the same internal component, with identical styling logic.
...
- The toggle's own tooltip/explanation needs to make clear *why* someone would want manual mode (a privacy/performance angle: not wanting every casually-visited site probed to see if it's a Cloudflare asset) — it can't be a bare on/off with no context.
